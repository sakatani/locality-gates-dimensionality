...
sizes still transfers nothing to larger ones --- even on a bounded-depth
task that its fixed iteration budget covers at every size), recurrent
message passing on
...
\paragraph{Set versus sequence.} \sloppy Permutation-invariant set
architectures are the established prior for order-irrelevant data
\citep{zaheer2017deepsets, lee2019set}. Our earlier pilot
(\S\ref{sec:boundary-global}) measured this gap directly on our own
relational tasks and found \emph{no additional} gain from 2D positional
structure under global attention --- the observation that motivated the
present locality-centered design.\par\fussy
...
locality provides. (Caveats: one recipe; larger-scale
global models remain untested, \S\ref{sec:limitations}.)

One confound remains in the F1 comparison: F1's required depth scales with
$W$, while the UT's iteration count is architecturally fixed at 16 by its
per-step embeddings, so its large-$W$ failure could partly reflect depth
budget rather than the operator. We therefore re-ran the identical recipe on
the bounded-depth families, where 16 iterations suffice at every evaluated
width (F4b needs $\sim K = 6$ propagation steps; F5 needs $T = 2$), under
pre-registered thresholds (3 seeds; raw data in
\texttt{a08b\_global\_bounded.json}). On F4b the UT trains to 0.96 at the
training widths --- above its F1 level --- and still transfers nothing:
0.86 at $W = 8$, 0.69 at $W = 12$, and chance (0.50--0.54) at
$W \in \{16, 20\}$, against the 2D-local arm's flat 0.93 at $W = 20$. Depth
budget is thereby eliminated as the explanation on this family: a trained
global model with ample iteration depth still cedes the transfer axis,
isolating the cause in the operator's lack of scale-transferable (local,
weight-shared) structure. On F5 the same recipe stopped just short of the
pre-registered training gate (0.78 against the 0.80 bar), so per protocol it
yields no transfer verdict; we report it as a training failure (for
completeness, its unseen-size accuracies show the same decay to chance).

A note on capacity asymmetry. The trained Universal Transformer uses hidden
width 64 --- twice the hidden width 32 of the local arms
(\S\ref{sec:arms}) --- because that capacity was needed to get the global
arm to train at all; the smaller, parameter-matched variant of our second
attempt did not reach a usable training accuracy. The asymmetry therefore
runs in the direction that favors the global arm, and its consequences cut
one way: the UT's in-distribution parity is achieved with a larger model
than the 2D-local arm it is compared against, and its transfer failure
cannot be attributed to capacity starvation. The comparison, if anything,
flatters the global arm; the transfer gap survives it.
...
48, hidden 64, 50 epochs, 20k mazes; 3 seeds, mean $\pm$ s.d.), the predicted
ordering emerges on the benchmark's own metric: the 2D-local arm solves
$11.3 \pm 5.5\%$ of held-out $9 \times 9$ mazes with $0.966 \pm 0.006$ pixel
accuracy and fades to zero by $13 \times 13$ ($1.3 \pm 0.9\%$ at
$11 \times 11$), while the matched 1D arm solves \emph{none at any size in
any seed} ($0.000$ full-maze throughout), its pixel accuracy stuck at the
all-background floor ($0.851$--$0.871$).
...
pilot scale was not achieved, and we report it as such. The zero-versus-nonzero
ordering is seed-stable: the 1D arm never solves a single held-out maze in
any of the three seeds, at any size.
...
\item \textbf{Task breadth.} The mechanism families are synthetic and
  ...
  batteries remain future work, and the maze check in particular, though now
  run at 3 seeds, has pilot-scale absolute performance (the ordering is
  seed-stable but neither arm masters the benchmark).
\item \textbf{The capable-global comparison is partially closed.} After two
  ...
  model at pilot scale --- with the depth-budget confound eliminated on the
  bounded-depth metric family (F4b), where the same trained model with
  ample iteration depth still decays to chance at unseen sizes. What
  remains untested is scale: whether much larger
